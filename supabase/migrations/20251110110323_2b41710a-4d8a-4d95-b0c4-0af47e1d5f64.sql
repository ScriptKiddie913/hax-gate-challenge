-- Update submit_flag to use lowercase table name
...
BEGIN
  ...
  -- Get hashed flag and salt from secure table (lowercase table name)
  ...
  -- Hash submitted flag using SHA256 and compare
  v_submitted_hash := encode(digest(v_salt || btrim(submitted_flag) || v_salt, 'sha256'), 'hex');
...
-- Update admin_set_flag to use lowercase table name  
...
BEGIN
  ...
  -- Insert or update hashed flag (lowercase table name)
...
-- Update admin_has_flag to use lowercase table name
