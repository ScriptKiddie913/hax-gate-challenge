-- Fix digest calls: convert text to bytea and cast algorithm to text
...
BEGIN
  ...
  -- Proper pgcrypto usage: convert text to bytea and cast algorithm to text
  v_submitted_hash := encode(digest(convert_to(v_salt || btrim(submitted_flag) || v_salt, 'UTF8'), 'sha256'::text), 'hex');
...
BEGIN
  ...
  -- Salt and hash using convert_to + digest
  v_salt := encode(digest(convert_to(p_challenge_id::text || extract(epoch from now())::text || random()::text, 'UTF8'), 'sha256'::text), 'hex');
  v_hash := encode(digest(convert_to(v_salt || btrim(p_flag) || v_salt, 'UTF8'), 'sha256'::text), 'hex');
